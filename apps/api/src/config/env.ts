...
// Placeholder values shipped in .env.example — never valid in production, even
// though they satisfy the shape/length checks below.
const KNOWN_PLACEHOLDER_SECRETS = new Set([
  'replace-me-with-openssl-rand-base64-32',
  'carnotea_dev_password',
]);
...
export const envSchema = z
  ...
  .superRefine((config, ctx) => {
    ...
    if (KNOWN_PLACEHOLDER_SECRETS.has(config.BETTER_AUTH_SECRET)) {
      ctx.addIssue({
        code: 'custom',
        path: ['BETTER_AUTH_SECRET'],
        message:
          'BETTER_AUTH_SECRET is still the .env.example placeholder; generate a real secret.',
      });
    }

    if (
      [...KNOWN_PLACEHOLDER_SECRETS].some((placeholder) =>
      ...
      ctx.addIssue({
        code: 'custom',
        path: ['DATABASE_URL'],
        message: 'DATABASE_URL still contains the .env.example placeholder password.',
      });
    }
  });
