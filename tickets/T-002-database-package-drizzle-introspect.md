...
- [ ] `packages/db/` exists as workspace package `@carnotea/db`.
- [ ] `packages/db/src/schema/` contains Drizzle TypeScript schema files
...
- [ ] `pnpm db:migrate` applied against a fresh Postgres, followed by running
      `sql/07_seed_data.sql` lookup inserts, results in all checks in
      `tests/integrity_audit.sql` returning `passed = true`.
- [ ] `packages/db/src/index.ts` exports a `createDb(databaseUrl)` factory
      returning a typed Drizzle client.
- [ ] `packages/db/AGENTS.md` documents the day-to-day workflow:
      edit schema → `pnpm db:generate` → review migration diff → `pnpm db:migrate`.
- [ ] Root `README.md` and `docs/getting-started.md` reference `pnpm db:migrate`
