...
| TanStack Router         | Routing                           | code-based; routes assembled in `apps/web/src/lib/router.ts` (see `apps/web/AGENTS.md`)                     |
| TanStack Query          | Server-state caching              | `QueryClient` in `apps/web/src/lib/queryClient.ts`; dev devtools tree-shaken in prod                        |
...
| i18next + react-i18next | i18n (pl + en)                    | bundled resources; browser detector persists choice to `localStorage`; [ADR-0007](./adr/0007-i18n-pl-en.md) |
